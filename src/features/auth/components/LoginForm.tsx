...
import { signInRequestSchema } from "@/core/dtos/auth.dto";
import { z } from "zod";
...
export default function LoginForm({ onSubmit }: Props) {
  ...
  function validateField(field: keyof ValidationErrors, value: string) {
    ...
    try {
      signInRequestSchema.parse(formData);
  ...
  function handleSubmit(e: FormEvent) {
    ...
    try {
      signInRequestSchema.parse({ email, password });
      setErrors({});
  ...
  return (
    <form
      ...
      className="w-full max-w-md bg-white/10 backdrop-blur-md border border-white/20 p-8 rounded-2xl shadow-2xl"
    >
      ...
      <button
        type="submit"
        className="w-full bg-brand-500 text-white py-2.5 rounded-lg hover:bg-brand-700 transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-brand-500 focus:ring-offset-2 focus:ring-offset-transparent mt-2 font-medium"
