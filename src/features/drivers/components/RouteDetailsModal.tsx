...
export default function RouteDetailsModal({ routeId, onClose }: { routeId: number; onClose: () => void }) {
  const { data: route, isLoading: routeLoading, isError: routeError } = useRouteById(routeId);
  const vehicleId = route?.vehicleId ?? null;
  const { data: vehicle, isLoading: vehicleLoading } = useVehicleById(vehicleId ?? undefined);
    ...
    <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-60">
      <div className="bg-white w-11/12 max-w-4xl p-4 rounded shadow-lg overflow-auto max-h-[80vh]">
        <div className="flex justify-between items-center mb-4">
          <h3 className="text-lg font-semibold">Route Details {routeId}</h3>
